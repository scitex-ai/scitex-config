...
This file also force-sets COVERAGE_PROCESS_START / COVERAGE_FILE at
module import time and drops an idempotent ``.pth`` shim into
site-packages so coverage data from any child Python interpreter
(``subprocess.run``, ``jupyter nbconvert --execute``, etc.) is captured.

``os.environ.setdefault`` would be a silent no-op here because
``pytest-cov`` has already set ``COVERAGE_FILE`` to a per-test tmp dir by
the time conftest is loaded — we must force-set both vars.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_config_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"

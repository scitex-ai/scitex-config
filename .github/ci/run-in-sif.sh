...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-config's —
...
# so only scitex-config's own thin dep set is fetched per run.
#
...
# scitex-config's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-config + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-config's historical bare-uv/pip workflow so a
...
# scitex-config's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-config style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-config-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-config's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_config --cov-report=xml --cov-report=term \
    -p no:cacheprovider

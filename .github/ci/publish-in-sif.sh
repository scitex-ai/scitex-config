...
# (project=scitex-config, owner=ywatanabe1989, repo=scitex-config,
...
TMPDIR="/tmp/publish-scitex_config-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
export TMPDIR
rm -rf "$TMPDIR"
...
echo "PUBLISH-OK: scitex-config dist/* uploaded to PyPI via manual OIDC trusted publishing"
